...
import type { ProvisionRunEvent } from '../shared/types';
...
/**
 * Tab live output untuk satu proses provisioning — struktur identik dengan
 * DeployView (xterm read-only, convertEol karena exec channel tanpa PTY),
 * cuma beda sumber event (provision:event) dan tidak ada konsep rollback
 * karena provisioning bukan tugas satu Project tertentu.
 */
...
export function ProvisionView({ sessionId, templateId, active, onExit }: ProvisionViewProps) {
  const { t } = useI18n();
  ...
  useEffect(() => {
    ...
    // Subscribe SEBELUM memanggil provision:run — sama alasannya dengan
    // DeployView: event step pertama bisa terpicu sinkron sebelum promise
    // run() resolve, jadi event yang datang lebih dulu ditampung di buffer.
    const buffered: ProvisionRunEvent[] = [];
    let knownRunId: string | null = null;

    const handleEvent = (event: ProvisionRunEvent) => {
      switch (event.type) {
        case 'stepStart':
    ...
    const unsubscribe = window.ssh.provision.onEvent((event) => {
      if (knownRunId) {
    ...
    const initialize = () => {
      ...
      void (async () => {
        try {
          const runId = await window.ssh.provision.run(sessionId, templateId);
          if (disposed) {
            window.ssh.provision.cancel(runId);
            return;
          }
    ...
    return () => {
      ...
      observer?.disconnect();
      unsubscribe();
      if (runIdRef.current) window.ssh.provision.cancel(runIdRef.current);
      if (opened) term.dispose();
      termRef.current = null;
      fitRef.current = null;
      runIdRef.current = null;
    };
  // sessionId+templateId saja: satu run dipicu sekali per mount, tidak diulang tiap render.
  ...
  if (error) {
    return (
      <div className="flex h-full items-center justify-center bg-abyss p-8 text-center">
        <div>
          <p className="text-sm text-coral">{t('provision.startFailed')}</p>
